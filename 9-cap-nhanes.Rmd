...
As part of the effort to tackle this challenge, we analyzed data from Third National Health and Nutrition Examination Survey (NHANES III) [@nhanes_1994] dataset and the accompanying National Death Index (NDI). The first goal of the analysis was to explore the available data and identify potential variables of interest for cancer mortality risk prediction. The second goal was to define a general approach for variable selection that can be applied to other large cohort studies that provide rich, multidimensional datasets. Studies of interest need not focus on cancer. For example, the data from the Atherosclerosis Risk in Communities (ARIC) study [@ARIC_1989] and the Framingham Heart Study [@Mahmood_2014] can be used in cancer research studies [@Kreger_1991; @Joshu_2017].
