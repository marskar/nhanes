...
**Results:** We fit 9600 Cox proportional hazards models with and without ridge regularization to randomly selected subsets of up to 48 variables and divided the models into 4 groups of roughly equal size according to their Akaike Information Criterion (AIC) and concordance values. Controlling for model size, highly significant variables (p < 1e-10) were most common in the group with the lowest AIC and highest concordance and least common in the group with highest AIC and lowest concordance. Interestingly, some of these highly significant variables appeared only in models with above-median corcordance values or only in ridge-penalized models with below-median AIC values. Based on variable description, we selected a subset (exact number) of the most frequent highly significant variables and trained a final model that performed well compared to the randomly generated models.
...
![Concordance (y-axis) and AIC (x-axis) values of 200 Cox models with (triangle) and without (circle) ridge penalties split into quadrants based on](img/1-quad.pdf){width=100%}

![Scatter plot showing the concordance (y-axis) and AIC (x-axis) values of 200 Cox models](img/2-volcano.pdf){width=100%}

![Scatter plot showing the concordance (y-axis) and AIC (x-axis) values of 200 Cox models](img/3-varbar.pdf){width=100%}
...
: Description of Highly Significant Variables
