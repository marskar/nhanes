...
allvrs1 <- as.name(paste(names(dat)[3:37], collapse=' + '))
allform1 <- update(form1, paste("~ ", allvrs1))
allfit1 <- svycoxph(formula = allform1, design = des, data = dat[train,])

allvrs2 <- as.name(paste(names(dat)[37:75], collapse=' + '))
allform2 <- update(form1, paste("~ ", allvrs2))
allfit2 <- svycoxph(formula = allform2, design = des, data = dat[train,])

ridvrs <- as.name(paste(names(dat)[3:12], collapse=', '))
ridform <- update(form1, paste("~ ridge(", ridvrs, ')'))
ridfit <- svycoxph(formula = ridform, design = des, data = dat[train,])

ridvrs <- as.name(paste(names(dat)[5:36], collapse=', '))
ridform <- update(form1, paste("~ ridge(", ridvrs, ')'))
ridfit <- svycoxph(formula = ridform, design = des, data = dat[train,])

ridvrs2 <- as.name(paste(names(dat)[37:75], collapse=', '))
ridform2 <- update(form1, paste("~ ridge(", ridvrs2, ')'))
ridfit2 <- svycoxph(formula = ridform2, design = des, data = dat[train,])

dim(dat)
ord
lasvrs <- as.name(paste(names(dat)[3:45], collapse=', '))
